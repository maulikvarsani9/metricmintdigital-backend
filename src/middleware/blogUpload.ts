...
/**
 * Middleware to process uploaded blog images (compress and convert to WebP)
...
export const getBlogImageUrl = (req: Request, filename: string): string => {
  const isProduction = process.env.NODE_ENV === 'production';
  const protocol = isProduction ? 'https' : req.protocol;
  const host = req.get('host');
  return `${protocol}://${host}/uploads/blog/${filename}`;
};
